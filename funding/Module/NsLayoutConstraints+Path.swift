...
extension UIView {
    ...
    public func relativeToSelf(_ constraints: [Constraint], relation: Constraint.Relation = .equal, priority: UILayoutPriority = .required ) -> [NSLayoutConstraint] {
        return relativeTo(self, positioned: constraints)
    }
    
    public func relativeToParent(_ constraints: [Constraint], relation: Constraint.Relation = .equal, priority: UILayoutPriority = .required ) -> [NSLayoutConstraint] {
        guard let safeSuperView = superview else { return [] }
        return relativeTo(safeSuperView, positioned: constraints)
    }
    
    public func relativeToUpperSibling(_ constraints: [Constraint], relation: Constraint.Relation = .equal, priority: UILayoutPriority = .required) -> [NSLayoutConstraint] {
        guard let safeSuperView = superview else { return [] }
        let index = safeSuperView.subviews.firstIndex(of: self) ?? 0
        let upperSibiling = safeSuperView.subviews[max(0, index - 1 )]
        return relativeTo(upperSibiling, positioned: constraints)
    }
    
    public func relativeToLowerSibling(_ constraints: [Constraint], relation: Constraint.Relation = .equal, priority: UILayoutPriority = .required) -> [NSLayoutConstraint] {
        guard let safeSuperView = superview else { return [] }
        let index = safeSuperView.subviews.firstIndex(of: self) ?? 0
        let upperSibiling = safeSuperView.subviews[min(safeSuperView.subviews.count - 1, index + 1 )]
        return relativeTo(upperSibiling, positioned: constraints)
    }
    
    public func applyToAllSubviews(_ constraints: [Constraint], relation: Constraint.Relation = .equal, priority: UILayoutPriority = .required ) -> [NSLayoutConstraint] {
        var resultingConstraints = [NSLayoutConstraint]()
        self.subviews.forEach { subView in
            resultingConstraints += subView.relativeTo(self, positioned: constraints)
        }
...
extension Constraint.Configuration {
    ...
    public static func width(margin: CGFloat) -> [Constraint] {
